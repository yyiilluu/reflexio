...
import { LayoutDashboard, Users, MessageSquare, ThumbsUp, CheckCircle, TrendingUp, TrendingDown } from "lucide-react"
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts"
import { useState, useEffect } from "react"
import { getDashboardStats, type DashboardStats, type TimeSeriesDataPoint } from "@/lib/api"
...
const getTimeBucket = (timestamp: number, granularity: string): number => {
  ...
  if (granularity === "daily") {
    ...
    return Math.floor(new Date(monday.getFullYear(), monday.getMonth(), monday.getDate()).getTime() / 1000)
  } else {
    // Start of month
...
const calculateTrend = (current: number, previous: number): { text: string; isPositive: boolean } => {
  if (previous === 0) {
      ...
      <CardContent>
        {isLoading ? (
          <div className="flex items-center justify-center h-[280px]">
            <div className={`animate-spin rounded-full h-10 w-10 border-2 border-transparent`} style={{ borderTopColor: color, borderRightColor: color }}></div>
          </div>
        ) : data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            ...
            >
              ...
              <YAxis
                ...
                axisLine={{ stroke: "#e2e8f0" }}
                tickLine={{ stroke: "#e2e8f0" }}
                label={yAxisLabel ? { value: yAxisLabel, angle: -90, position: 'insideLeft', fill: "#64748b" } : undefined}
              />
              <Tooltip
                contentStyle={{
...
export default function Dashboard() {
  const [interactionsGranularity, setInteractionsGranularity] = useState<"daily" | "weekly" | "monthly">("daily")
  const [profilesGranularity, setProfilesGranularity] = useState<"daily" | "weekly" | "monthly">("daily")
  const [feedbacksGranularity, setFeedbacksGranularity] = useState<"daily" | "weekly" | "monthly">("daily")
  const [evaluationsGranularity, setEvaluationsGranularity] = useState<"daily" | "weekly" | "monthly">("daily")
      ...
      <div className="p-8">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4 mb-8">
          {isLoading ? (
            ...
            Array.from({ length: 4 }).map((_, i) => (
              <Card key={i} className={`border bg-gradient-to-br ${statConfigs[i].bgGradient} ${statConfigs[i].borderColor}`}>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <div className="h-4 bg-white/60 rounded w-24 animate-pulse"></div>
                  <div className={`h-10 w-10 rounded-xl ${statConfigs[i].iconBg} opacity-50 animate-pulse`}></div>
                </CardHeader>
                <CardContent>
              ...
              return (
                ...
                >
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <div className={`h-10 w-10 rounded-xl ${stat.iconBg} flex items-center justify-center shadow-lg`}>
                  ...
                  <CardContent>
                    ...
                    <div className={`flex items-center gap-1 mt-3 text-sm font-semibold ${stat.trend.isPositive ? 'text-emerald-600' : 'text-red-500'}`}>
                      ...
                      <span className="text-slate-400 font-normal text-xs ml-1">vs last period</span>
                    </div>
                  </CardContent>
                </Card>
              )
        ...
        <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
          <ChartCard
            ...
            data={dashboardData ? groupTimeSeriesData(dashboardData.interactions_time_series, interactionsGranularity, "average") : []}
          ...
          <ChartCard
            ...
            data={dashboardData ? groupTimeSeriesData(dashboardData.profiles_time_series, profilesGranularity, "average") : []}
          ...
          <ChartCard
            ...
            data={dashboardData ? groupTimeSeriesData(dashboardData.feedbacks_time_series, feedbacksGranularity, "average") : []}
          ...
          <ChartCard
            ...
            data={dashboardData ? groupTimeSeriesData(dashboardData.evaluations_time_series, evaluationsGranularity, "average") : []}
