...
export default function SettingsPage() {
  ...
  const updateAnthropicConfig = (updates: Partial<AnthropicConfig>) => {
    setConfig({
      ...config,
      api_key_config: { ...config.api_key_config, anthropic: { api_key: config.api_key_config?.anthropic?.api_key || "", ...config.api_key_config?.anthropic, ...updates } },
    })
  }
  ...
  const handleOpenAIModeChange = (mode: "direct" | "azure") => {
    ...
    if (mode === "direct") {
      ...
      updateOpenAIConfig({ api_key: undefined, azure_config: { api_key: "", endpoint: "", api_version: "2024-02-15-preview" } })
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (config.storage_config.type === "supabase") {
      ...
      if (!sc.url || !sc.key || !sc.db_url) {
        const missing = [!sc.url && "Supabase URL", !sc.key && "Supabase Key", !sc.db_url && "Database URL"].filter(Boolean)
  ...
  const updateStorageConfig = (updates: Partial<StorageConfig>) => {
    setConfig({ ...config, storage_config: { ...config.storage_config, ...updates } as StorageConfig })
  }
  ...
  const changeStorageType = (type: StorageType) => {
    if (originalConfig.storage_config.type === type) {
      ...
      const newSC: StorageConfig = type === "local" ? { type: "local", dir_path: "./data" } : { type: "supabase", url: "", key: "", db_url: "" }
  ...
  const addProfileExtractor = () => {
    setConfig({
      ...config,
      profile_extractor_configs: [...config.profile_extractor_configs, { id: generateId(), extractor_name: "", profile_content_definition_prompt: "", context_prompt: "", metadata_definition_prompt: "", manual_trigger: false }],
    })
  }

  const updateProfileExtractor = (id: string, updates: Partial<ProfileExtractorConfig>) => {
    setConfig({ ...config, profile_extractor_configs: config.profile_extractor_configs.map(pec => pec.id === id ? { ...pec, ...updates } : pec) })
  }

  const removeProfileExtractor = (id: string) => {
    setConfig({ ...config, profile_extractor_configs: config.profile_extractor_configs.filter(pec => pec.id !== id) })
  }
  ...
  const addRequestSourceToExtractor = (extractorId: string, source: string) => {
    ...
    if (ext) updateProfileExtractor(extractorId, { request_sources_enabled: [...(ext.request_sources_enabled || []), source] })
  }

  const removeRequestSourceFromExtractor = (extractorId: string, sourceIndex: number) => {
    const ext = config.profile_extractor_configs.find(pec => pec.id === extractorId)
    if (ext?.request_sources_enabled) updateProfileExtractor(extractorId, { request_sources_enabled: ext.request_sources_enabled.filter((_, idx) => idx !== sourceIndex) })
  }
  ...
  const addAgentFeedback = () => {
    setConfig({
      ...config,
      agent_feedback_configs: [...config.agent_feedback_configs, { id: generateId(), feedback_name: "", feedback_definition_prompt: "", feedback_aggregator_config: { min_feedback_threshold: 2, refresh_count: 2 } }],
    })
  }

  const updateAgentFeedback = (id: string, updates: Partial<AgentFeedbackConfig>) => {
    setConfig({ ...config, agent_feedback_configs: config.agent_feedback_configs.map(afc => afc.id === id ? { ...afc, ...updates } : afc) })
  }

  const removeAgentFeedback = (id: string) => {
    setConfig({ ...config, agent_feedback_configs: config.agent_feedback_configs.filter(afc => afc.id !== id) })
  }
  ...
  const addRequestSourceToFeedback = (feedbackId: string, source: string) => {
    ...
    if (fb) updateAgentFeedback(feedbackId, { request_sources_enabled: [...(fb.request_sources_enabled || []), source] })
  }

  const removeRequestSourceFromFeedback = (feedbackId: string, sourceIndex: number) => {
    const fb = config.agent_feedback_configs.find(afc => afc.id === feedbackId)
    if (fb?.request_sources_enabled) updateAgentFeedback(feedbackId, { request_sources_enabled: fb.request_sources_enabled.filter((_, idx) => idx !== sourceIndex) })
  }

  // --- Agent Success ---
  const addAgentSuccess = () => {
    setConfig({ ...config, agent_success_configs: [...config.agent_success_configs, { id: generateId(), evaluation_name: "", success_definition_prompt: "" }] })
  }

  const updateAgentSuccess = (id: string, updates: Partial<AgentSuccessConfig>) => {
    setConfig({ ...config, agent_success_configs: config.agent_success_configs.map(asc => asc.id === id ? { ...asc, ...updates } : asc) })
  }

  const removeAgentSuccess = (id: string) => {
    setConfig({ ...config, agent_success_configs: config.agent_success_configs.filter(asc => asc.id !== id) })
  }

  // --- Tools ---
  const addTool = () => {
    setConfig({ ...config, tool_can_use: [...(config.tool_can_use || []), { tool_name: "", tool_description: "" }] })
  }

  const updateTool = (toolIndex: number, updates: Partial<ToolUseConfig>) => {
    if (config.tool_can_use) {
      setConfig({ ...config, tool_can_use: config.tool_can_use.map((tool, idx) => idx === toolIndex ? { ...tool, ...updates } : tool) })
    }
  }

  const removeTool = (toolIndex: number) => {
    if (config.tool_can_use) {
      setConfig({ ...config, tool_can_use: config.tool_can_use.filter((_, idx) => idx !== toolIndex) })
    }
  }
      ...
      <div className="p-8">
        <div className="max-w-[1800px] mx-auto">
          ...
          {hasUnsavedChanges && saveStatus !== "saving" && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3 shadow-sm" role="status" aria-live="polite">
          ...
          {errorMessage && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl flex items-center gap-3 shadow-sm" role="alert">
          ...
          {saveStatus === "success" && (
            <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3 shadow-sm" role="status" aria-live="polite">
            ...
            <TabsContent value="general" className="mt-6">
              <div className="space-y-6">
                ...
                <ExtractionParamsSection
                  ...
                  onWindowStrideChange={(v) => setConfig({ ...config, extraction_window_stride: v })}
                />
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <DialogTitle className="text-xl font-semibold text-slate-800">Discard Changes</DialogTitle>
            </div>
            <DialogDescription className="text-sm text-slate-600 pt-2">
              Are you sure you want to discard all unsaved changes? This will revert the configuration to the last saved state.
            </DialogDescription>
          </DialogHeader>
