...
export const inferStorageConfig = (storageConfig: any): StorageConfig => {
  ...
  if ("dir_path" in storageConfig) {
    ...
    return { type: "supabase", url: storageConfig.url, key: storageConfig.key, db_url: storageConfig.db_url }
  }
  return { type: "local", dir_path: "" }
}
...
export const configsAreEqual = (config1: Config, config2: Config): boolean => {
  if (JSON.stringify(config1.storage_config) !== JSON.stringify(config2.storage_config)) return false
  if ((config1.agent_context_prompt || "") !== (config2.agent_context_prompt || "")) return false
  if (JSON.stringify(config1.tool_can_use || []) !== JSON.stringify(config2.tool_can_use || [])) return false
  if (config1.extraction_window_size !== config2.extraction_window_size ||
      config1.extraction_window_stride !== config2.extraction_window_stride) return false
