...
export default function CustomStorageEdge({
  ...
  const labelStyle = (data as { labelStyle?: { fontSize: number; fontWeight: number; fill: string } })?.labelStyle || {
    fontSize: 12,
    fontWeight: 700,
    fill: "#1d3557",
  }

  const labelBgStyle = (data as { labelBgStyle?: { fill: string; fillOpacity: number; stroke: string; strokeWidth: number } })?.labelBgStyle || {
    fill: "white",
    fillOpacity: 0.95,
