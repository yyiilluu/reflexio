...
import { ReactFlow, Node, Edge, Controls, Background, BackgroundVariant, MarkerType } from "@xyflow/react"
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    allExtractors.forEach((extractor, idx) => {
      ...
      if (extractor.type === "profile") {
        ...
        if (profileConfig.request_sources_enabled && profileConfig.request_sources_enabled.length > 0) {
